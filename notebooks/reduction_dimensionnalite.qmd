...
print(df.isna().sum())
df.jauge_oslo_imputee = df.jauge_oslo.apply(lambda x: 1 if np.isnan(x) else 0)
df.puissance_administrative_imputee = df.puissance_administrative.apply(lambda x: 1 if np.isnan(x) else 0)
...
p = umap.plot.points(mapper, values=np.log(df_raw.nombre_prescriptions_majeurs_hist), cmap='RdYlGn_r', background='white')
umap.plot.show(p)
p = umap.plot.points(mapper, values=df_raw.longueur_hors_tout, cmap='RdYlGn_r', background='white')
umap.plot.show(p)
p = umap.plot.points(mapper, values=np.log(df.jauge_oslo), cmap='RdYlGn_r', background='white')
umap.plot.show(p)
p = umap.plot.points(mapper, values=df.jauge_oslo_imputee, cmap='RdYlGn_r', background='white')
umap.plot.show(p)
...
* L'îlot 1 et 4 sont quasiment sans prescription majeures
* L'îlot 2 est constitué de navires avec des faibles valeurs de prescriptions majeures 
