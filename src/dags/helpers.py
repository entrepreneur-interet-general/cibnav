...
def embulk_filepath():
    return os.path.join(base_path(), "etl", "embulk", "oracle_to_postgres.yml.liquid")
...
def embulk_run(dag, script, table, env=None, task_id=None):
    return BashOperator(
        ...
        bash_command="EMBULK_QUERY=$(cat \"/opt/etl/sql/{table}.sql\" | tr '\n' ' ') {bin} {file}".format(
            table=table, bin=embulk_bin(), file=embulk_filepath()
        ),
        dag=dag,
        env=resolve_env(env),
    )
...
def read_sql_query(filename):
    filepath = os.path.join(
        base_path(), "etl", "sql", "{filename}.sql".format(filename=filename)
    )
...
def numero_immatriculation(val, secret):
    hashed = sha1(
        "{secret}{immatriculation}".format(secret=secret, immatriculation=val).encode(
            "utf-8"
        )
    ).hexdigest()
    return hashed
