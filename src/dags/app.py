...
from helpers import default_args, embulk_run, read_sql_query, resolve_env, connection_db
...
def creation_cibles(df: pd.DataFrame):
    df['cible'] = df['nombre_prescriptions_majeurs'].apply(lambda x: '1' if x > 0 else '0').astype(int)
    df['cible'] = df['cible'] | df['nombre_prescriptions'].apply(lambda x: '1' if x >= PRESCRIPTIONS_THRESHOLD else '0').astype(int)
    return df
...
def creation_visite_simulee(visites):
    ...
    for navire in visites['id_nav_flotteur'].unique():
        ligne = {
                ...
                'annee_construction': visites[visites['id_nav_flotteur'] == navire]['annee_construction'].max() ,
                'nombre_moteur': visites[visites['id_nav_flotteur'] == navire]['nombre_moteur'].max() ,
                'nombre_prescriptions_majeurs': 0,
        }
        for param in CAT_PARAM + NUM_PARAM:
            if param not in ['nombre_prescriptions_hist', 'nombre_prescriptions_majeurs_hist', 'age', 'sitrep_history', 'delai_visites', 'annee_visite']:
                ligne[param] = visites[visites['id_nav_flotteur'] == navire][param].max()
...
def chargement_dataset(engine, database="dataset_train", index="id_gin_visite", prediction_phase=False):
...
def train():
    ...
    model_pipe = Pipeline([
            ...
            ('RF', RandomForestClassifier(n_estimators=200, max_depth=110, min_samples_split=8, min_samples_leaf=3, max_features=3, bootstrap=True))
    ])
...
def prediction_flotte():
    ...
    previsions.to_sql('score_v3', engine, if_exists='replace',)
